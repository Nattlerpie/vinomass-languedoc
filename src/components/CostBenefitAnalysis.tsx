  ...
  const comparisonData: CostBenefitData[] = [
    {
      ...
      saf: safRevenue,
      difference: safRevenue - regionalData.traditionalRevenue,
    ...
    },
    {
      ...
      difference: regionalData.operatingCosts - regionalData.traditionalCosts,
    ...
    },
    {
      ...
      traditional: regionalData.traditionalRevenue - regionalData.traditionalCosts,
      saf: safProfit,
      difference: safProfit - (regionalData.traditionalRevenue - regionalData.traditionalCosts),
    ...
    },
    {
      ...
      traditional: 0,
      saf: co2Avoided / 1000, // Convert to kt
      difference: co2Avoided / 1000,
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-white/95 backdrop-blur-sm border-wine-burgundy/20 shadow-elegant">
        ...
        <CardContent>
          {viewType === 'comparison' && (
            <div className="space-y-6">
               {/* Summary Cards with Regional Data */}
                        ...
                        <tr key={index} className="border-b border-wine-cream/50">
                          ...
                          <td className="text-right py-2 text-wine-charcoal">
                            {item.category.includes(t('cost.benefit.jobs.created')) || item.category.includes(t('cost.benefit.co2.avoided')) 
                              ? item.traditional 
                              : `€${item.traditional.toFixed(1)}M`}
                          </td>
                          <td className="text-right py-2 text-wine-charcoal">
                            {item.category.includes(t('cost.benefit.jobs.created')) || item.category.includes(t('cost.benefit.co2.avoided'))
                              ? item.saf.toFixed(0)
                              : `€${item.saf.toFixed(1)}M`}
                          </td>
          ...
          {viewType === 'breakdown' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {[
                  { 
                    label: t('cost.benefit.direct.jobs'), 
                    value: `${Math.round((currentData.wasteAllocation?.realisticJobs || (regionId === 'champagne' ? 150 : 350)) * 0.6)} ${t('cost.benefit.positions')}`, 
                    impact: t('cost.benefit.production.maintenance') 
                  },
                  { 
                    label: t('cost.benefit.indirect.jobs'), 
                    value: `${Math.round((currentData.wasteAllocation?.realisticJobs || (regionId === 'champagne' ? 150 : 350)) * 1.8)} ${t('cost.benefit.positions')}`, 
                    impact: t('cost.benefit.services.suppliers') 
                  },
                  { 
                    label: t('cost.benefit.local.taxes'), 
                    value: `€${(safRevenue * 0.04).toFixed(1)}M/an`, 
                    impact: t('cost.benefit.municipal.revenue') 
                  },
                  { 
                    label: t('cost.benefit.local.purchases'), 
                    value: `€${Math.round(safRevenue * 0.13)}M/an`, 
                    impact: t('cost.benefit.circular.economy') 
                  }
                ].map((item, index) => (
